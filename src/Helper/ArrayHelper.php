...
class ArrayHelper
{
    ...
    public static function sortBy($items, string $key, string $direction = 'ASC'): array
    {
        ...
        foreach ($items as $k => $element) {
            try {
                $sortedArray[$k] = $accessor->getValue($element, $key);
            } catch (Exception $e) {
                $sortedArray[$k] = 'undefined';
            }
        }

        if ('ASC' === $direction) {
            uasort($sortedArray, static function ($a, $b) {
                return $a >= $b;
            });
        } else {
            uasort($sortedArray, static function ($a, $b) {
